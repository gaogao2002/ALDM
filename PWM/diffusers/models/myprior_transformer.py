...
class MyPriorTransformer(ModelMixin, ConfigMixin):
    """
    ...
    def __init__(
        ...
        self.cond_encoder = MLP(in_dim=1280, hidden_dim=embedding_dim//2, out_dim=embedding_dim)
        self.cond_encoder1 = MLP(in_dim=1280, hidden_dim=embedding_dim//2, out_dim=embedding_dim)
        self.cond_encoder2 = MLP(in_dim=1280, hidden_dim=embedding_dim//2, out_dim=embedding_dim)
        ...
        self.encoder_hidden_states_proj2 = nn.Linear(embedding_dim, inner_dim)
    ...
    def forward(
        ...
        encoder_hidden_states2: torch.FloatTensor,
        ...
        encoder_hidden_states2 = self.encoder_hidden_states_proj2(encoder_hidden_states2)
        ...
        if test_flag:
            ...
            encoder_hidden_states2 = torch.cat([negative_prompt_embeds, encoder_hidden_states2])


        hidden_states = torch.cat(
            [
                encoder_hidden_states,    # cloth-mask             
                encoder_hidden_states1,   # 分割图
                encoder_hidden_states2,   # deeppose      
                proj_embeddings,          # cloth
